...
function(git_do_all)
    set(options)
    set(oneValueArgs BRANCH PATH PROJECT_ID VAR KEY DST)
    set(multiValueArgs)
    ...
    set(key "${OPTS_KEY}")
    if ("${OPTS_DST}" STREQUAL "")
        set(destdir "${import_remote_path}/${key}")
    else()
        set(destdir "${OPTS_DST}")
    endif()
...
function (import_remote input)
    ...
    set(oneValueArgs TAG ALIAS CMAKE DST)
    set(multiValueArgs)
    cmake_parse_arguments(OPTS "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
    ...
    if ("${previous_branch}" STREQUAL "")
        ...
        git_do_all(
            ...
            KEY "${remote_key}" PROJECT_ID "${remote_path}" DST "${OPTS_DST}"
            VAR dest
        )
        set("${remote_alias}_path" "${dest}" CACHE STRING "" FORCE)
        set(cmake_path "${OPTS_CMAKE}")
        if ("${cmake_path}" STREQUAL "")
            set(cmake_path "${import_remote_default_cmake_root}")
        endif()
        if (NOT "${OPTS_NO_CMAKE}")
            add_subdirectory("${dest}/${cmake_path}" "${import_remote_bin_path}/${remote_key}" EXCLUDE_FROM_ALL)
        endif()
    else()
